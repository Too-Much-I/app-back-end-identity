...
- 현재 단계: 브랜치 `feat/TMI-103-firebase-withdrawal-lifecycle`에서 Jira `TMI-103` Stage 1 구현을 완료했다. Firebase/SNS fresh proof 탈퇴, withdrawal lifecycle, 원자적 User·Session·eligibility·lifecycle 처리, cleanup pending 로그인 gate와 기본 비활성 activation flag를 반영했고 전체 505개 테스트가 통과했다. Jira 댓글·상태 전환은 수행하지 않았다
- 상태 기준일: 2026-08-25
