...
Signature Algorithm은 RS256이다. Identity Service만 RSA Private Key를 보유하며, 외부 서비스에는 Private Key를 배포하지 않는다.
...
- Identity Service는 JWKS endpoint를 제공한다.
...
JWKS endpoint의 실제 배포 URL과 허용할 `issuer` 값은 환경별 설정으로 관리하며, 실제 Key나 Secret을 이 문서에 기록하지 않는다.
