...
public final class PhoneEligibilityRetryPolicy {
	...
	public PhoneEligibilityRetryPolicy(DoubleSupplier jitterSource) {
	...
	public Duration delay(int attemptCount) {
		...
		long uncappedMillis = Math.multiplyExact(BASE.toMillis(), multiplier);
		long cappedMillis = Math.min(uncappedMillis, CAP.toMillis());
