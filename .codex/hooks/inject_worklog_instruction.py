...
def main() -> None:
        ...
        emit(
            {
                "systemMessage": (
                    ...
                    "WORKLOG와 CURRENT_STATE는 수동으로 갱신해야 합니다."
                ),
                "hookSpecificOutput": {
    ...
    instruction = (
        ...
        "1. docs/codex/WORKLOG.md의 과거 기록을 건드리지 말고 파일 끝에 "
        "이번 작업 항목을 append하세요.\n"
        "2. docs/codex/CURRENT_STATE.md를 이번 작업 결과에 맞게 갱신하세요.\n"
        "3. 새 WORKLOG 항목에 현재 turn_id marker를 정확히 포함하세요: "
        f"{marker}\n"
        "4. Secret, Token, Password, 실제 Key, 전체 MongoDB URI를 기록하지 "
        "마세요."
    )
    emit(
        {
